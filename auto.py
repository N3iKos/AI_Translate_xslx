...
PRESETS = [
    {
        ...
        "label": "Formal / Resmi",
        "prompt": (
            "Translate into a formal, professional tone. "
            "Use proper grammar and honorifics where appropriate. "
    ...
    },
    {
        ...
        "prompt": (
            "Translate in a natural, conversational tone. "
    ...
    },
    {
        ...
        "prompt": (
            "Translate game text formally. "
            "CRITICAL: Preserve ALL placeholders EXACTLY as-is: "
            "{0} {1} {2} {name} {item} %s %d %i %f %u %l %g "
            "<tag> </tag> [tag] \\n \\t \\r \\\\n \\\\t and any similar patterns. "
            "Do NOT translate, move, or modify placeholders. "
            "Use formal language suitable for game UI and menus."
        ),
    },
    {
        "key": "game_casual",
        ...
        "prompt": (
            "Translate game text casually and engagingly. "
            "CRITICAL: Preserve ALL placeholders EXACTLY as-is: "
            "{0} {1} {2} {name} {item} %s %d %i %f %u %l %g "
            "<tag> </tag> [tag] \\n \\t \\r \\\\n \\\\t and any similar patterns. "
            "Do NOT translate, move, or modify placeholders. "
            "Use natural, engaging language for game dialogue."
        ),
    },
    {
        "key": "technical",
        "label": "Technical / IT",
        "prompt": (
            "Translate technical/IT content accurately. "
            "Preserve technical terms, acronyms, code snippets, and product names. "
    ...
    },
    {
        ...
        "prompt": (
            "Translate as subtitle/dialogue. "
            "Keep translations concise and natural-sounding when spoken. "
    ...
    },
    {
        ...
        "prompt": (
            "Translate product descriptions for e-commerce. "
            "Make text compelling and marketable. "
            "Preserve product names, specifications, and numbers."
        ),
    },
    {
        "key": "custom",
...
def call_llm(provider_key, api_key, model, prompt_text):
    if provider_key == "gemini":
        return _gemini_generate(api_key, model, prompt_text)
    elif provider_key == "groq":
        return _openai_chat(
...
def parse_batch_response(text, expected_row_nums):
    ...
    for i, rn in enumerate(expected_row_nums):
        if i < len(lines):
            line = lines[i]
            # Strip any leading number prefix if present
            m2 = re.match(r'^\[?\d+\]?[.):]?\s*(.*)', line)
            result3[rn] = m2.group(1).strip() if m2 and m2.group(1).strip() else line
    return result3
...
class ProcessingEngine:
    ...
    def run_translate(self, source_rows, context_map=None):
        ...
        for b_idx, batch in enumerate(batches):
            ...
            if with_repair and repair_col and translated:
                repaired = self._repair_batch_after_translate(batch, translated, context_map)
                if repaired:
    ...
    def run_translate_parallel(self, source_rows, context_map=None, n_workers=1):
        ...
        def process_one_translate(batch, key, wave_idx, slot_idx, total_b):
            """Single translate task — runs in its own thread."""
            ...
            repair_result = {}
            if with_repair and repair_col and translated:
                repair_result = self._repair_batch_after_translate(
                    batch, translated, context_map
                )
                if repair_result:
                    with self._write_lock:
                        write_batch(out_path, repair_col, repair_result)
                    with self._print_lock:
                        log_write(f"Repair    → col {get_column_letter(repair_col)}  "
                                  f"rows {batch[0][0]}-{batch[-1][0]}")
